...
print('There are {} users and {} movies in this dataset.'.format(
...
number_reviews = 75
popular_movies = list(
    set(movie_ratings_count.query('count >= @number_reviews').index))
...
user_reviews = 210
popular_users = list(
    set(movie_ratings_count.query('count >= @user_reviews').index))
filtered_user_df = filtered_movie_df[filtered_movie_df.movieId.isin(
    popular_users)]
...
movie_title_index = {
    ...
    enumerate(list(movies_df.set_index(
        'movieId').loc[df_movie_pivot.index].title))
}
...
def make_recommendation(model_knn, data, fav_movie, mapper, n_recommendations):
    ...
    ), distances.squeeze().tolist())), key=lambda x: x[1])[:-1:0]
    # get reverse mapper
